...
### 1. Chuẩn bị (Setup)
- [ ] Tổ chức mã nguồn: tạo cấu trúc thư mục mới có `plugins/`.
- [ ] Soạn file `requirements.txt` cập nhật (cần: `livekit-agents`, `edge-tts`, `openai`, `pydub`/`av` cho decode audio...).
- [ ] Đảm bảo hiểu rõ bản chất của file cũ `AI_handoff_context.md` và `main.py` để kéo các hàm có ích (như danh sách từ bị cấm - hallucination blacklist, prompt, vv) sang kiến trúc mới.
...
- [ ] Sử dụng `silero.VAD.load()` cho phát hiện giọng nói.
- [ ] Lệnh định nghĩa `AgentServer` và `@server.rtc_session`.
- [ ] Gắn kết STT, LLM, TTS vào đối tượng `VoicePipelineAgent` (hoặc cấu hình kiểu `AgentSession`).
- [ ] Xử lý Event bắt **Room Metadata** (để setup Source, Target Language lúc Client mới connect).
- [ ] Đăng ký Event Channel để bắn (Publish) text nhận diện tiếng nguồn (STT text) và text đích (LLM text) qua DataChannel cho FE.

### 4. Triển khai Google Colab
- [ ] Tạo file Notebook `run_colab.ipynb` hướng dẫn và cung cấp bash script để setup Cuda, clone Repo, gắn `ngrok` (nếu cần), cài thư viện và run agent.
